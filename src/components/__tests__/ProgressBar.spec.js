...
describe('ProgressBar.vue', () => {
  ...
  test('set the bar to 100% width when finish is called', () => {
    const wrapper = shallowMount(ProgressBar)
    wrapper.vm.start()
    wrapper.vm.finish()
  ...
  test('increase width by 1% every 100ms after start call', () => {
    ...
    expect(wrapper.element.style.width).toBe('60%')
  })

  test('clears timer when finish is called', () => {
    jest.spyOn(window, 'clearInterval')
    setInterval.mockReturnValue(1234)
    const wrapper = shallowMount(ProgressBar)
    wrapper.vm.start()
    wrapper.vm.finish()
  ...
  test('add error class when fail is called', () => {
    const wrapper = shallowMount(ProgressBar)
    wrapper.vm.start()
    wrapper.vm.finish()
    wrapper.vm.fail()
  ...
  test('set the bar to 100% width when fail is called', () => {
    const wrapper = shallowMount(ProgressBar)
    wrapper.vm.start()
    wrapper.vm.fail()
    expect(wrapper.element.style.width).toBe('100%')
